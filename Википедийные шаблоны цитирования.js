{
	"translatorID": "609ff7fb-6002-42b7-bd2d-de5e5b007d6a",
	"translatorType": 2,
	"label": "Википедийные шаблоны цитирования",
	"creator": "Le Loi",
	"target": "txt",
	...
	"inRepository": true,
	"lastUpdated": "2020-07-14 11:45:52"
}
	...
	while (item = Zotero.nextItem()) {
		...
		if (item.creators && item.creators.length) {
			if (type == "Cite episode") {
				...
				if (item.creators.length) {
					properties["автор"] = formatAuthors(item.creators)
				}
		...
		if (item.itemType == "bookSection") {
			...
			properties["заглавие"] = item.title;
		...
		if (item.place) {
			if (type == "Cite episode") {
				...
				properties["место"] = item.place;
			}
		}
		
		if (item.series) {
			properties["серия"] = item.series;
		} else if (item.seriesTitle) {
			properties["серия"] = item.seriesTitle;
		} else if (item.seriesText) {
			properties["серия"] = item.seriesText;
		}
		...
		if (item.url && item.accessDate) {
			if (item.itemType == "bookSection") {
				...
				properties["ссылка"] = item.url;
			}
		}
		
		if (properties.pages) {
			properties["страницы"] = properties.pages.replace(/[^0-9]+/,"–"); // separate page numbers with en dash
		}
